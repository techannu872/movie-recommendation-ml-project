import requests
from requests.exceptions import RequestException

def fetch_poster(movie_id):
    try:
        response = requests.get(f'https://api.themoviedb.org/3/movie/{movie_id}?api_key=0020784a601e67841ecfe402a731dcd0&language=en-US')
        response.raise_for_status()
        data = response.json()
        return "https://image.tmdb.org/t/p/w500/" + data.get('poster_path', '')
    except RequestException as e:
        print(f"Error fetching poster for movie ID {movie_id}: {e}")
        return None


import streamlit as st
import pickle 
import pandas as pd




def recommend(movie):
    movie_index = movies[movies['title'] == movie].index[0]
    distances = similarity[movie_index]
    movies_list = sorted(list(enumerate(distances)), reverse=True, key=lambda x:x[1])[1:6]
    
    recommended_movie = []
    recommended_movie_poster = []
    recommend_url = []
    for i in movies_list:
        movie_id = movies.iloc[i[0]].movie_id
        movie_var = movies.iloc[i[0]].title
        recommended_movie.append(movie_var)
        #fetch poster from API
        poster_a= fetch_poster(movie_id)
        if poster_a == None :
            poster_a = 'data:image/png;base64,'
        recommended_movie_poster.append(poster_a)
        movie_url=''
        
        for i in range(len(movie_var)):
            if movie_var[i] == ' ' or movie_var[i] == ',':
                movie_url += '+'
            else:
                movie_url += movie_var[i]

        google_url = f'https://www.google.com/search?q={movie_url}'
        recommend_url.append(google_url)
        
    return recommended_movie, recommended_movie_poster, recommend_url
    
st.title('Movie Recommender System')

movies_dict = pickle.load(open('movie_dict.pkl', 'rb'))
movies = pd.DataFrame(movies_dict)
similarity = pickle.load(open('similarity.pkl','rb'))

selected_movie_name = st.selectbox(
    'How would you like to interact with the recommender system?',
    movies['title'].values
    )

if st.button('Recommend'):
    names,poster,urls = recommend(selected_movie_name)
    print(urls)
    col1, col2, col3, col4, col5 = st.columns(5)
    with col1:
        st.text(names[0])
        st.markdown(f"[![{names[0]}]({poster[0]})]({urls[0]})", unsafe_allow_html=True)
    with col2:
        st.text(names[1])
        st.markdown(f"[![{names[1]}]({poster[1]})]({urls[1]})", unsafe_allow_html=True)
    with col3:
        st.text(names[2])
        st.markdown(f"[![{names[2]}]({poster[2]})]({urls[3]})", unsafe_allow_html=True)
    with col4:
        st.text(names[3])
        st.markdown(f"[![{names[3]}]({poster[3]})]({urls[3]})", unsafe_allow_html=True)
    with col5:
        st.text(names[4])
        st.markdown(f"[![{names[4]}]({poster[4]})]({urls[4]})", unsafe_allow_html=True)